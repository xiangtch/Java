...
class Character implements java.io.Serializable, Comparable<Character> {
    ...
    * Strong bidirectional character type "AL" in the Unicode specification.
     ...
     * @since 1.5
     */
    // unicode的合理取值范围现在为：0x0000-0x10ffff
     ...
     * @since  1.5
     */
    // codePoint - 0x10000; 取高 10 位 + D800; 取低 10 位 + DC00
    ...
    static int codePointAtImpl(char[] a, int index, int limit) {
        char c1 = a[index];
        // 判断是否要高位代理，因为低位代理的值比高位代理的值大
        // 如果是高位代理，则跟低位代理一起组成 unicode 码
        // 但是如果索引的是低位代理的话，就只返回低位代理对应的 unicode 码了
     ...
     * @since   1.7
     */
    // 高位代理规则：高位的 10 bit 值加上 D800 得到第一个码元或称作高位代理
    // 原始公式应该是: ((codePoint - MIN_SUPPLEMENTARY_CODE_POINT) >>> 10) +  MIN_HIGH_SURROGATE
    // 即 codePiont - 0x10000, 然后取高 10 位，再加上 D800
    // 这里由于 0x10000 的低 10 位全为 0，所以 (codePoint - 0x10000) >>> 10 跟 codePoint >>> 10 - 0x10000 >>> 10 是一样的
     ...
     */
    // 0x3fff : 1023
    // utf-16 低位代理规则：低位的 10 bit 的值加上 0xDC00 得到第二个码元或称作低位代理
    ...
    //  0xffff(0x10000-0x10ffff) 之后的字符代理规则：码位减去 0x10000 得到一个长度为 20 bit 的二进制 0-0xFFFF,
    // 高 10 位 加上 0xD800 得到第一个码元或称作高位代理
    // 低 10 位 加上 0xDC00 得到第二个码元或称作低位代理
     ...
     * subarray 子数组的开始位置(在原数组上面的索引)
     * @param count the length of the subarray in {@code char}s (子数组的长度)
     * @param index the index to be offset 指定的索引位置
     * @param codePointOffset the offset in code points codePoint 的偏移量
     * @return the index within the subarray 返回子数组中的索引
     * @exception NullPointerException if {@code a} is null.
     ...
     *
     * @param a 原始 char 数组
     * @param start 子数组开始位置在原始数组中的索引
     * @param count 子数组的长度
     * @param index 指定的数组的索引位置
     * @param codePointOffset codePoint 偏移量
     * @return 返回的值为原数组中的索引的位置
     */
    static int offsetByCodePointsImpl(char[]a, int start, int count,
        ...
        if (codePointOffset >= 0) {
            ...
            for (i = 0; x < limit && i < codePointOffset; i++) {
                // 如果有高于 BMP 的字符，即采用了两个 char 编码的, x 要多 + 1
                if (isHighSurrogate(a[x++]) && x < limit &&
                    isLowSurrogate(a[x])) {
                    x++;
                }
            }
            // 如果 i < codePointOffset 抛出数组越界异常
            if (i < codePointOffset) {
                throw new IndexOutOfBoundsException();
            }
        } else {
            ...
            for (i = codePointOffset; x > start && i < 0; i++) {
                // codePointOffset < 0 的时候，对于采用了两个 char 编码的要多 - 1
     ...
     * @see     Character#isDigit(char)
     */
    // 将 char 类型的 ch 按照指定的 radix 进制转换成 int 类型的值
    public static int digit(char ch, int radix) {
        // 将 ch 转成 int(codePoint) 来进行进制的转换
