...
public class BigInteger extends Number implements Comparable<BigInteger> {
     ...
     * @serial
     */
    // BigInteger 对象的符号标致 signum: 1- 正，0- 0, -1- 负
    final int signum;
     ...
     */
    // 存入字节，采用 big-endian 顺序，即高位有效数字存进低位地址，低位有效数字存入高位地址
    // 就是顺序存储二进制，0 的时候 mag 的长度为 0
    final int[] mag;
    ...
    */
     ...
     */
    // 根据数据量级以及符号构造 BigInteger 对象
    BigInteger(int[] magnitude, int signum) {
        // 如果 magnitude 的长度为 0，则 signum 返回 0, 否则返回 signum
        this.signum = (magnitude.length == 0 ? 0 : signum);
        this.mag = magnitude;
        // 如果 mag 的长度超过定义的 BigInteger 的最大长度
        if (mag.length >= MAX_MAG_LENGTH) {
            // 检查边界条件
            checkRange();
        }
    ...
    private void checkRange() {
        // 如果 mag 的长度超过定义的最大长度，或者等于定义的最大长度且最高位小于 0 ，即二进制编码的最高位为 1
     ...
     */
    // 根据 long 类型的值返回一个 BigInteger 对象
    public static BigInteger valueOf(long val) {
        ...
        if (val > 0 && val <= MAX_CONSTANT)
            // 返回 posConst[val] 位置的 BigInteger 对象
            return posConst[(int) val];
        // 如果 -16 <= val < 0，则直接返回 negConst 对应位置缓存的 BigInteger 对象
        else if (val < 0 && val >= -MAX_CONSTANT)
            return negConst[(int) -val];

        // 否则根据 val 构造一个 BigInteger 对象
     ...
     */
    // 根据指定的 val 的值构造一个 BigInteger 对象
    private BigInteger(long val) {
        // 如果 val < 0, 则将 val 转成对应的正数，signum 标记为 -1
        if (val < 0) {
            val = -val;
            signum = -1;
        } else {
    ...
    // 初始化一个长度为 MAX_CONSTANT + 1 的值为正数的 BigInteger[]
    private static BigInteger posConst[] = new BigInteger[MAX_CONSTANT+1];
    // 初始化一个长度为 MAX_CONSTANT + 1 的值为负数的 BigInteger[]
     ...
     */
    // 内存可见，任何一个线程对 volatile 变量的修改，对于其他线程来讲都是可见的。
    // 即线程每次获取到的 volatile 的值都是最新的，相当于轻量级的 sychronized
    ...
    static {
        for (int i = 1; i <= MAX_CONSTANT; i++) {
            ...
            // 将数组的第一个元素的值设为 i
            magnitude[0] = i;
            // 创建一个 BigInteger 对象缓存到 posConst[i] 位置
            posConst[i] = new BigInteger(magnitude,  1);
            // 创建一个 BigInteger 对象缓存到 negConst[i] 位置
         ...
         * on demand.
         */
        // 创建一个 powerCache 二维数组，一维长度为最大的进制位 + 1
        powerCache = new BigInteger[Character.MAX_RADIX+1][];
        // 创建一个 logCache 数组，数组长度为最大进制位 + 1
        logCache = new double[Character.MAX_RADIX+1];

        for (int i=Character.MIN_RADIX; i <= Character.MAX_RADIX; i++) {
            // 给 powerCache 的每一维数组的第一个位置放入对应的进制数
            powerCache[i] = new BigInteger[] { BigInteger.valueOf(i) };
            // logCache 对应位置放入对应的对数值
     ...
     * @return {@code mag << n}
     */
    // 返回一个将 mag 里面对应二进制左移 n 位之后二进制对应的数组
