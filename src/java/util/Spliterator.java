...
public interface Spliterator<T> {
     ...
     */
    // 执行一个元素的操作，也就是 action 里面的操作，如果有元素则返回 true，没有元素返回 false
     ...
     * @throws NullPointerException if the specified action is null
     */
    // forEach 遍历执行所有元素的 action 里面的动作
     ...
     */
    // 如果这个集合能够被分割，返回一个分割迭代器，即 Spliterator
     ...
     */
    // 返回元素的估计数量，可以用 forEachRemaining 来获得，如果由于元素太多或者不知道元素数量，返回 Long 里面的 MAX_VALUE
    long estimateSize();
     ...
     * else {@code false}
     */
    // 如果 Spliterator 迭代器包括所给的所有特征，则返回 true
     ...
     */
    /* 如果这个 Spliterator 迭代器的元素是通过 Comparator 排好序的 SORTED 元素，就返回 Comparator
     * 如果元素不是 SORTED 的，则抛出错误
      * */
     ...
     */
    // 特征值表示为元素定义遇到顺序，表示迭代器需要按照其原始顺序迭代其中的元素
     ...
     */
    // 特征值表示对于每对遇到的元素 x,y，!x.equals(y)，即表示迭代器中的元素是没有重复的
     ...
     */
    // 特征值表示为遇到的顺序遵循定义的排序顺序，即表示迭代器是按照某种方式排序的顺序迭代其中元素的
     ...
     */
    // 特征值表示从 estimateSize() 遍历或分割之前返回的值，表示有限大小，在没有结构源修改的情况下，表示完全遍历将遇到元素数量的精确计数
    // 即表示迭代器将要迭代的元素的个数是可计数的，有界的
     ...
     */
    // NONNULL 特征值表示源保证遇到的元素不为 null，即表示迭代器迭代的元素是没有 null 的
     ...
     */
    // IMMUTABLE 特征值表示元素源不能在结构上进行修改，也就是说不能添加，替换或删除元素，因此在遍历过程中不会发生这种改变
    // 即表示迭代器迭代的元素是不可变的，也不可以增加、替换和删除
     ...
     */
    // CONCURRENT 特征值表示可以通过多个线程安全同时修改元素源（允许添加，替换或删除），而无需外部同步
    // 即表示迭代器的数据源是线程安全的
     ...
     */
    // SUBSIZED 特征值表示由 trySplit() 产生的所有 Spliterators 都将被 SIZED 和 SUBSIZED
    // 表示当前迭代器所有的子迭代器（直接或者间接的），都是 SIZED 和 SUBSIZED 的
